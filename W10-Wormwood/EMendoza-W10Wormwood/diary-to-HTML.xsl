...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:p"> [<xsl:value-of select="concat('page ',translate(@n, 'page0', ''))"/>] <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <p>
            <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="d:name"> <!-- Enclosing in HTML span element with class attribute so CSS formatting can be applied. -->
        <span class="name"><xsl:apply-templates/> (<xsl:value-of select="@role"/>)</span>
    </xsl:template>
    ...
    <xsl:template match="d:group">
        <span class="group"><xsl:apply-templates/></span>
    </xsl:template>
    
    <xsl:template match="d:book">
        <span class="book"><xsl:apply-templates/></span>
    </xsl:template>
    
    <xsl:template match="d:profanity">
        <span class="profanity"><xsl:apply-templates/></span>
    </xsl:template>
    
    <xsl:template match="d:bible">
