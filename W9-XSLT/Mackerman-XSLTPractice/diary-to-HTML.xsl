...
<!--!!!!!!!!!1LIST OF NAMES DIV!!!!!!!!!!!!!!!!
                ...
                <div>
                    ...
                    <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                      <li>
                        <xsl:value-of select="."/> <xsl:value-of select="upper-case(@role)"/> <!--display role in uppercase--> 
                      </li>
                        </xsl:for-each>
                    </ol>
                    </p>
    ...
    <xsl:template match="d:cite">
        <a> <!--hyperlink tag-->
            <xsl:attribute name="href"> <!--match to citation provided in xml-->
                <xsl:value-of select="@ref"/> <!--extract!-->
            </xsl:attribute>
       <xsl:apply-templates/> 
        </a>
    </xsl:template>
...
<!--    <xsl:template match="d:cite"/>
    ...
    <xsl:template match="d:q"> 
        "<xsl:apply-templates/>" (<a href="{cite/@ref}"><xsl:value-of select="cite"/></a>
    </xsl:template>-->
