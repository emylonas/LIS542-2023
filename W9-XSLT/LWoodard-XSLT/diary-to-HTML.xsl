...
<!-- Step 4: The transformation generates a list of names at the end of the HTML document. Add a list of places. -->
    ...
    <!-- Step 1: Display the paragraph numbers at the beginning of the paragraph, enclosed in square brackets.--> 
    ...
    <!-- Step 2: The Biblical quotation which is at around line 71 in the Adams file would look nice with quotation marks around it. Write a template that adds quotation marks before and after the content of the <q> element.  -->
    <xsl:template match="d:q">
        "<xsl:apply-templates/>"
    </xsl:template>

    <!-- Step 3:In the Adams file, the <cite> element has a @ref attribute whose value is a URL linking to the quoted Bible passage. Write a template that matches <cite> and turns the @ref into an HTML <a> element (HTML link). -->
    <xsl:template match="d:cite">
        <a href="https://www.kingjamesbibleonline.org/Titus-3-5/"><xsl:apply-templates/></a> 
    </xsl:template>
    
    <!-- See above near "list of names" for Step 4 -->
    
    <!-- Step 5: Surprise me! Maybe add roles, either to the index of names or to the names as they appear in the diary. If you are familiar with CSS, you can modify that as well. -->
   
    <!-- For this one, I just don't know where I went wrong exactly. I think because select is a boolean attribute,it can only show whether the name HAS a role, and not what the role is. I can select the @role attribute, but not display it.-->
