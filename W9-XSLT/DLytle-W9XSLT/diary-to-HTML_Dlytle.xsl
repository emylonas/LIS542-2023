...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                                                <li>
                                                                      <xsl:value-of select="."/> (<xsl:value-of select="@role"/>);  <xsl:value-of select="@place"/>
                                                                    </li>
                                                            </xsl:for-each>
                        </ol>
                    </p>
    ...
    <xsl:template match="d:q"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <q>
            <xsl:apply-templates/>
        </q>
    </xsl:template>
    ...
    <xsl:template match="d:cite"> 
        ...
        <a href="."><xsl:value-of select="ref"/>
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="d:pb"> [<xsl:value-of select="concat('page ',translate(@n, 'page0', ''))"/>] </xsl:template>
