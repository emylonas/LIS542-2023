...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <p>
            [<xsl:value-of select="@n"/>] <!-- I have a feeling that string is involved here, but I am very scared to make it pretty :')-->
            <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="d:cite">
        <a href="@ref">King James Bible </a> <xsl:apply-templates/>
    </xsl:template>
