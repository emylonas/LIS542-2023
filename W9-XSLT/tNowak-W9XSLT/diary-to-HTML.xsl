...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:p">[<xsl:value-of select="@n"/>]  <!-- Match <p>, surround each one with an HTML <p> element and keep going  -->
        <p>
            <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="d:cite">
        <a href="{@ref}">3 Tit. 5<xsl:apply-templates/></a>
    </xsl:template>
    ...
    <xsl:template match="d:q">"<xsl:value-of select="."/>"<!-- enclose q element with quotes --></xsl:template>
