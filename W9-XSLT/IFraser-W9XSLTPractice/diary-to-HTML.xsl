...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html>
            ...
            <body>
                ...
                <div>
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                                                <li>
                                                                      <xsl:value-of select="."/> (<xsl:value-of select="@role"/>) <!-- Problem 5: adds roles to names in list -->
                                                                    </li>
                                                            </xsl:for-each>
                    ...
                    <p>
                        <ol>
                                     <xsl:for-each select="//d:entries//d:place" xml:space="preserve"> <!-- Problem 4: Create list of Places -->
                                              <li>
                                                    <xsl:value-of select="."/>
                                                  </li>
                                          </xsl:for-each>
                    ...
                    <p>
                        <ol>
                            <xsl:for-each select="//d:entries//d:name//@role" xml:space="preserve">
                                                                <li>
                                                                      <xsl:value-of select="."/> <!-- Problem 5 Part 2: Decided to make a roles list too! -->
                                                                    </li>
                                                            </xsl:for-each>
                        </ol>
                    </p>
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <p>
            <xsl:choose>
                <xsl:when test="(substring(@n,2,1)) = '0'">
                    <xsl:value-of select="concat('[',substring(@n,3),']',' ')"/> <!-- Problem 1 Advanced: Removed leading zeros -->
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat('[',substring(@n,2),']',' ')"/>  <!-- Problem 1:Display numbers at beginning, enclosed in square brackets. Also added a space to improve the visual appeal (having the number right against the first word looked weird. -->
                </xsl:otherwise> 
            </xsl:choose>
            <xsl:apply-templates/>
        </p>
    ...
    <xsl:template match="d:q">
        <xsl:value-of select= "concat('&quot;',//d:q,'&quot;')"/> <!-- Problem 2: Put quotation marks around Bible quote. -->
    </xsl:template>
    
    <xsl:template match="d:cite">
        <a href="{@ref}"><xsl:apply-templates/></a> <!-- Problem 3 -->
    </xsl:template>
