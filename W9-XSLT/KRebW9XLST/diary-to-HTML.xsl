...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html>
            ...
            <body>
                ...
                <div>
                    ...
                    <h2>List of Places</h2> <!-- Modification #4: List of Places -->
                    <p>
                        <o1>
                    ...
                    <h3>List of Roles</h3> <!-- Modification #5: List of Roles, tried to make it appear following the modifications above but it will not link to the roles. Any input on this would be helpful!-->
                    <p>
                        <o1>
                                                       <xsl:for-each select="d:name//d:author//d:metadata//d:place//@role" xml:space="preserve">
                                                                <li>
                                                                     <xsl:value-of select="."/> (<xsl:value-of select="@role"/>)
                                                                    </li>
                                                            </xsl:for-each>
    ...
    <xsl:template match="d:cite"> <!-- Attempt at Modifcation #3 - I am having trouble getting the hyperlink to show up. -->
    ...
    <xsl:template match="d:pb"> [<xsl:value-of select="concat('page',translate(@n,'page 0',''))"/>] </xsl:template> <!--Modification #1 -->
    
    <xsl:template match="d:q"> "<xsl:value-of select="."/>" </xsl:template> <!--Modification #2 -->
   
   <xsl:template match="d:cite"><xsl:value-of select="@ref"/></xsl:template> 
    ...
    </xsl:template>https://www.kingjamesbibleonline.org/Titus-3-5/-->
