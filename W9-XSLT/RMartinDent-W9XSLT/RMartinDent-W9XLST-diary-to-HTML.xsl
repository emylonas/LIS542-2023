...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <p>
            [<xsl:value-of select="@n"/>] <!--I think this is the line of code that adds the display with square brackets for the paragraph numbers. I'm not sure how to make it look pretty, unfortunately. If I were working directly with the HTML code, I could add color or resize the text, but I'm not sure how to tell XLST to do that automatically.-->
            <xsl:apply-templates/>
        </p>
    </xsl:template>
