...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                                                <li><xsl:value-of select="."/> (<xsl:value-of select="@role"/>)</li> <!-- JH changed the li placement to get rid of weird spacing  -->
                                                            </xsl:for-each>
                        </ol>
                    </p>
                </div>
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:place" xml:space="preserve">
                                                                <li><xsl:value-of select="."/></li> <!-- JH changed the li placement to get rid of weird spacing  -->
                                                            </xsl:for-each>
                        </ol>
                    </p>
                </div>
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <p>
            [<xsl:value-of select="translate(@n, 'p0', '')"/>] <xsl:apply-templates/> <!-- JH this works fine until you get to "p10" where it makes it "1". I don't know how to specifically replace character place 1 in XSLT (assuming the "p" is in 0 position) -->
        </p>
    </xsl:template>
    
    <xsl:template match="d:q"> <!-- JH adding quotations around the q element, but with the way the xml is written, it will also include the citation which looks weird to me.  -->
    ...
    <xsl:template match="d:entry/d:date">
        ...
        <hr/> <!-- JH I added this for readability.  -->
        <h2>
            <xsl:apply-templates/>
        </h2>
    </xsl:template>
    ...
    <xsl:template match="d:cite"> <!-- JH I had to delete the other d:cite rule for this to work.  -->
            <a href="{@ref}">
                <xsl:apply-templates/>
            </a>
    </xsl:template>
    ...
    <xsl:template match="d:pb"> 
        <center><b>[<xsl:value-of select="concat('page ',translate(@n, 'page0', ''))"/>] </b></center>
    </xsl:template>
