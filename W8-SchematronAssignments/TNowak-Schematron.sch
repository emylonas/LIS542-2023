...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="starts-with(@n, 'poem')">n attributes in div elements must start with
                'poem'</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:l/@n">
            <assert test="string-length(.) = 4">The value of n in element l must be 4 digits long.
                For example n="000x" or n="00xx"</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:revisionDesc/tei:change">
            <assert test="string-length() ne 0">The change element must contain text</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:div/@type">
            <assert test=". = 'sonnet' or . = 'limerick'">The value of @type must be 'sonnet' or
                'limerick'</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:div[@type='sonnet']//descendant::tei:l">
            <assert test="@rhyme">line elements in sonnets must contain a @rhyme attribute</assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <!-- NOTE: I AM HAVING TROUBLE WITH THIS QUESTION. As this my second rule is written below, it fires an error both for <l> elemens which are NOT followed by <lb> AND it fires for <l> elements which are immediately followed by <lb> -->

    <pattern> 
        <rule context="tei:l">
            <assert test="tei:lb"> The "l" element must contain an "lb" element</assert>
        </rule>
        <rule context="tei:l/*[1]">
            <assert test="tei:lb">"lb" must follow an "l" element</assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:revisionDesc/tei:change/@who">
            <assert test="string-length() eq 2">@who can only contain 2 initials</assert>
        </rule>
    </pattern>
