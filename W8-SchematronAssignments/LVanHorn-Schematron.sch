...
<pattern>
    <rule context="tei:l">
        <assert test ="count(lb) lt 1">An l element must include an lb element</assert>
    </rule>
    ...
    <pattern>
        <rule context="tei:div/@n">
            <assert test="starts-with(., 'poem')">An n attribute must begin with the string "poem."</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:l/@n">
            <assert test="string-length() ge 3">Numbers in the n attribute must contain at least 3 digits.</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:RevisionDesc/tei:change">
            <assert test="string-length() ne 0">Change elements must include content.</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:div/@type">
                <assert test="contains(. , 'limerick')">Type attributes within the div element must include either sonnet or limerick as a value.</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="count(@rhyme) = 1">l elements must have rhyme attributes.</assert>
        </rule>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:lg">
            <assert test="contains(l,lb)">Each l element must include an lb element.</assert>
        </rule>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:head">
            <assert test="string-length() ne 0">Header elements must include the poem's title and author.</assert>
        </rule>
