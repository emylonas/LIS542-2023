...
<pattern>
    <rule context = "l">
        <assert test = "lb">Missing "lb" element.</assert>
    </rule>
    ...
    <pattern>
        <rule context = "div">
            <assert test = "@n = concat('poem',*)">The "n" attribute should begin with "poem".</assert>
        </rule>
    ...
    <pattern>
        <rule context = "*[@n]">
            <assert test = "@n = ****">Numbers must be four digits.</assert>
        </rule>
    ...
    <pattern>
        <rule context = "RevisionDesc/change">
            <assert test = "count(change)>0">"Change" element has no content.</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context = "div">
            <assert test = "@type = sonnet||limerick">"Type attribute is neither "sonnet" nor "limerick".</assert>
        </rule>
    ...
    <pattern>
        <rule context = "div[@type = sonnet]">
            <assert test = "@rhyme">Type "sonnet" does not contain attribute "rhyme"</assert>
        </rule>
        <rule context = "l">
            <assert test = "concat(text(), lb)">"l" element does not end in element "lb".</assert>
        </rule>
    </pattern>
