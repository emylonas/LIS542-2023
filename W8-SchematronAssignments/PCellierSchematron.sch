...
<pattern>
    <rule context="tei:l">
        <assert test="tei:lb">
            All "l" elements must contain an "lb" element
        </assert>
    </rule>
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="starts-with(@n, 'poem')">
                "n" attributes in "div" elements must begin with the string "poem"
            </assert>
        </rule>
    </pattern>
    ...
    <!-- 
        ...
        PETER NOTE: The text this question from the Canvas assignment page reads: "Write a rule to flag line numbers in the poem that are not contain 4 digits.
That is, n="0001" is correct. n="1" or n="02" are wrong" - I believe that is meant to read "...that DO not contain 4 digits," so I have written the rule to require that the @n element in the <l> element must always contain 4 characters.
    -->
    
    <pattern>
        <rule context="tei:l">
            <assert test="string-length(@n) = 4">
                All "n" attributes within an "l" element must be 4 digits 
            </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:revisionDesc/tei:change">
            <report test="string-length() = 0">
                "change" elements must contain content
            </report>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:div/@type">
            <assert test="contains(., 'sonnet') or contains(., 'limerick')">
                "n" attributes in "div" elements must contain either "sonnet" or "limerick"
            </assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:div[@type='sonnet']//tei:l">
            <assert test="@rhyme">
                "l" elements within sonnets must contain a "rhyme" attribute
            </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="child::node() = tei:lb">
                "l" elements in poems must be immediately followed by an "lb" element
            </assert>
        </rule>
   ...
   <!-- 
       ...
       PETER NOTE: This rule should flag when a poem of the <div> type "limerick" does not have exactly 5 lines
   -->
   
    <pattern>
        <rule context="tei:div[@type='limerick']//tei:lg">
            <report test="count(tei:l) ne 5">
                Limericks must contain exactly 5 lines
            </report>
        </rule>
    </pattern>
