...
<pattern>
    <rule context="tei:l">
        <assert test="tei:lb">'l' element must contain 'lb' element</assert>
    </rule>
</pattern>
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="starts-with(@n, 'poem')">@n attribute on div element must being with "poem."</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:l/@n">
            <assert test="string-length() eq 4">@n attribute for "l" element must contain at least 4 digits.</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:div/@type">
            <assert test="contains(.,'sonnet') or contains(.,'limerick')">@type attribute on div element must be equal to "sonnet" or "limerick."</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:div[@type='sonnet']/tei:lg/tei:l">
            <assert test="@rhyme">There must be a @rhyme attribute on 'l' element if div @type=sonnet.</assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:lg/tei:l">
            <assert test="node()[1] =tei:lb">'l' element must be immediatly followed by 'lb' element.</assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
       <rule context="tei:revisionDesc/tei:change">
           <assert test="@who">@who atrribute on 'change' element is required.</assert>
       </rule>
    </pattern>
