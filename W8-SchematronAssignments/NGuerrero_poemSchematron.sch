...
<pattern>
    <rule context="tei:l">
        <assert test="tei:lb">"l" element must contain an "lb" element within it.</assert>
    </rule>  
</pattern>
    ...
    <pattern>
        <rule context="tei:div/@n">
            <assert test="starts-with(.,'poem')">@n attribute must begin with the string "poem"</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:l/@n">
            <assert test="string-length()=4">Line numbers must be written with four digits (i.e. n="0001")</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:change">
            <assert test="string-length() ne 0">"change" element must have content</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:div[@type='sonnet']//descendant::tei:l">
            <assert test="@rhyme">Line in sonnets must have a rhyme attribute</assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="tei:lb">"l" element must contain an "lb" element within it.</assert>
            <assert test="*[1]='tei:lb'">The first child of each line element ("l") should be "lb"</assert>
        </rule>
    </pattern>
   ...
   <!-- 
       ...
       RULE: <l> elements must have text inside of it
   -->
   
    <pattern>
        <rule context="tei:l">
            <assert test="string-length() ne 0">Lines cannot be empty, they must have some content</assert>
        </rule>
    </pattern>
