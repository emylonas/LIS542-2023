...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern>
        <rule context="tei:div[@n]"> 
            <assert test="starts-with(@n,'poem')">
                The @n attribute must begin with the string, "poem."
            </assert>
        </rule>        
    </pattern> 
    ...
    <pattern>
        <rule context="tei:l[@n]">
            <assert test="string-length(@n) ge 3">
                @n attributes in l elements must have at least 3 digits!
            </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:revisionDesc/tei:change">
            <assert test="string-length(.) gt 0">
                The change element cannot be empty.
            </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:div[@type]">
            <assert test="contains(@type,'limerick') or contains(@type,'sonnet')">
                The @type attribute within the div element must contain "sonnet" or "limerick."
            </assert>
        </rule>
    </pattern>
    ...
    <!-- JH = I wasn't able to figure out #6, #7, and #8 due to time, but everything above does work in the poems.xml document. -->
    <pattern>
        <rule context="tei:div[@type='sonnet']/tei:l">
            <assert test="count(@rhyme) eq 1">
                l elements with @type='sonnet' must have a @rhyme attribute!
            </assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="contains(tei:l,tei:lb)">
                l elements must contain an lb element!
            </assert>
        </rule>
